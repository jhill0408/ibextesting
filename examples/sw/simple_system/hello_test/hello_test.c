...
int main(int argc, char **argv) {
  ...
  if (test4 < remainder || test4 >= remainder) {
...
for (k=0; k < M/NUMCORES; k = k + 512/NUMCORES){
for (n=0; n < (512/NUMCORES); n++) {
  if (n+k < M/NUMCORES) {

    int edges = s_index[n+firstrow+1] - s_index[n+firstrow];
    for(m=0; m < edges; m++){
       int core_dest = row2core(invcol_index[s_index[n+firstrow]+m]);
       if (core_dest != test4) {
       core_dest = core_dest << 16;
       core_dest = core_dest + (n+firstrow%512) + 4; // technically need to do % stuff, e.g. b[16] shoul dbe sent to x1  
    MOVE_GPRF_TO_MPRF(x2, core_dest); 
    MOVE_GPRF_TO_MPRF(x3, b[n])

    dest = 3;
    addrr = 2;
    SEND_MPRF(dest, addrr);
       } 
    }
  }
}
...
if (test4 == 0) {
  int token_dest = (1<<16) + 1;
  int token = 1;
  MOVE_GPRF_TO_MPRF(x1, token);
  MOVE_GPRF_TO_MPRF(x2, token_dest);
  dest = 1;
  addrr = 2;
  SEND_MPRF(dest, addrr);
  token = 0;
  ...
  if (test4 == NUMCORES - 1) {
    ...
    */
} ///////////////////////////////////////// EXTRA, GET RID OF THIS WHEN UNCOMMENTING ABOVE
