...
echo "set_property -name {STEPS.SYNTH_DESIGN.ARGS.MORE OPTIONS} -value {-generic TOPO=$TOPO_bp N=$CLIENTS D_W=$DW} -objects [get_runs synth_1]" >> test.tcl
...
luts=$(awk '/CLB\ LUTs/{print}' fc_${TOPO}_area_$CLIENTS.txt | head -n 1 | cut -d"|" -f 3 | awk '{print $1}')
